...
public class CommonWebView extends WebView implements IJSCallback {
    ...
    public void callbackJS(String callback, JSONObject result) {
        if (!TextUtils.isEmpty(callback)) {
            if (result != null)
                loadUrl("javascript:" + callback + "('" + replaceSingleQuot(result.toString()) + "')");
            else
    ...
    public void setOverScrollMode(int mode) {
        //fix crash:"android.webkit.WebViewFactory$MissingWebViewPackageException: Failed to load WebView provider: No WebView installed
        try{
    ...
    // 把字符串中的单引号变成  \'
    public String replaceSingleQuot(String str) {
        if (str == null)
            return str;
